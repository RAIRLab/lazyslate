...
import { setStateFromJSON, proofNodes, createLink, deleteLink, deleteNode, createNode, proofLinks } from "./state.js";
// Global Canvas State =================================================================================================
...
window.addEventListener('load', function () {
    ...
    if (urlParams.has("proof")) {
        ...
        const jsonProofString = LZString.decompressFromEncodedURIComponent(compressedProof);
...
function onKeyDown(event) {
    if (document.getElementById("new-node-menu").style.display == "none") { //Only process if the menu is hidden
        if (selectedNode != null && (event.key === "Backspace" || event.key === "Delete")) { //Delete node
            ...
            let newName = selectedNode.name == selectedNode.id.toString() ? "" : selectedNode.name;
...
function drawNode(node) {
    ...
    const width = 2 * padding + Math.max(ctx.measureText(node.expression.toString()).width, ctx.measureText(node.name).width, ctx.measureText(assumptionsString).width);
...
//# sourceMappingURL=proofCanvas.js.map